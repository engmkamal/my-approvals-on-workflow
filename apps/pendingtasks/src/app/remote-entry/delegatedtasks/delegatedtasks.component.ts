...
@Component({
  selector: 'portal-delegatedtasks',
  templateUrl: './delegatedtasks.component.html',
  styleUrls: ['./delegatedtasks.component.scss'],
})
export class DelegatedtasksComponent implements OnInit, AfterViewInit {
  ...
  delegatedUsers:any = [];
  //private delegatedUsers:Observable<any>;
  ...
  async getDeligets(){
    return new Promise((resolve, reject)=>{
      const dt = (new Date()).toISOString();
      const apiUrl = `https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('Task%20Delegation')/items?&$top=5&$select=GUID,ID,User/ID,FormDate,ToDate,Delegate_x0020_To/ID,Delegate_x0020_To/Title,Modified&$expand=User/ID,Delegate_x0020_To/ID&$filter=(Delegate_x0020_To/ID eq '${this.logedInUser.aDId}') and(ToDate le '${dt}')&$orderby=Created desc`;
