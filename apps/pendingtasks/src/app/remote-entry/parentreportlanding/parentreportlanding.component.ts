...
export class ParentreportlandingComponent implements OnInit, AfterViewInit {
  ...
  async getRowData_Old(i:any){
    ...
    const detListname = i.config.ApprovalListInfo[0].name;
    const detSelQry = i.config.ApprovalListInfo[0].select;
    ...
    if (Object.prototype.hasOwnProperty.call(this.dashboardsListsInfo[i.listIndex].ApprovalListInfo[0], 'primaryKey') && this.dashboardsListsInfo[i.listIndex].ApprovalListInfo[0].primaryKey != "") {
      detTblKeyFld = this.dashboardsListsInfo[i.listIndex].ApprovalListInfo[0].primaryKey;
    }
  ...
  async getRowData(i:any) {
    ...
    // let mstApiUrl = "https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('" + mstListname + "')/items?&$top=1000&$select=" + mstSelQry + "";
    // let detApiUrl = "https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('" + detListname + "')/items?&$top=1000&$select=" + detSelQry + "";
    let mstApiUrl = `https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('${mstListname}')/items?&$top=1000&$select=${mstSelQry} `; 
    let detApiUrl = `https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('${detListname}')/items?&$top=1000&$select=${detSelQry}&$filter=Author/ID eq '${this.logedInUser.aDId}' `; 
    ...
    return new Promise((resolve, reject)=>{ 
      if(this.logedInUser.access != 'NoAccess'){
        ...
        if(this.logedInUser.office != "Corporate"){
          mstApiUrl = `https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('${mstListname}')/items?&$top=1000&$select=${mstSelQry} `; 
          detApiUrl = `https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('${detListname}')/items?&$top=1000&$select=${detSelQry}&$filter=Author/ID eq '${this.logedInUser.aDId}' `; 
          // mstApiUrl = `https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('${mstListname}')/items?&$top=1000&$select=${mstSelQry}&$filter=substringof('${this.logedInUser.office}', Author/Office ) `; 
          // detApiUrl = `https://portal.bergerbd.com/leaveauto/_api/web/lists/getByTitle('${detListname}')/items?&$top=1000&$select=${detSelQry}&$filter=substringof('${this.logedInUser.office}', Author/Office ) `; 
        }
          try {
            //==========implementing forkJoin ============
            from(forkJoin({
    ...
    return new Promise((resolve, reject)=>{
      this._actRoute.paramMap.subscribe((url:any) => {
        ...
        if (wfName) { 
          ...
          this.displayText = this.dashboardsListsInfo[wfIndex].DisplayTxt;
        }
        else{
